export const yatesCorrection = [
  ...
  { type: "heading", text: "小於 5 指的是哪個數字？" },
  { type: "paragraph", text: "傳統教材常把 Yates 校正的使用時機簡記成：2×2 表的觀察總數 N≤20，或 A、B、C、D 其中一個觀察次數小於 5。這個寫法容易記住『資料很少時不要直接相信未校正的卡方近似』，但它只是早期的快速篩選規則。" },
  { type: "callout", tone: "caution", label: "正式判斷要回到期望次數", text: "Pearson 卡方近似是否可靠，直接相關的是 H₀ 下的期望次數 E=列合計×欄合計÷N。觀察次數小於 5 不一定代表期望次數也小於 5；反過來，總樣本數看似不少，也可能因列、欄比例極不平衡而出現很小的期望次數。" },
  { type: "paragraph", text: "因此，原先的 N≤20 或某格觀察次數小於 5 可以當成提醒，接著仍要計算四格期望次數。若期望次數稀疏，Fisher 精確檢定通常比硬套卡方近似更直接；若使用 Yates，則要知道它仍然是連續卡方分布的近似修正。" },
